...
// in which case the maxRuns cap can never trigger. If maxRuns is smaller, the
// function stops after emitting maxRuns runs and entries beyond the last emitted
// run are left unbatched (their indices are not covered by any returned run).
inline uint32_t BuildBatchRuns(BatchEntry* entries, uint32_t count,
    ...
    std::sort(entries, entries + count, [](const BatchEntry& a, const BatchEntry& b) {
        ...
        return a.entity < b.entity; // total order -> deterministic per-run entity order
    });
    uint32_t runCount = 0;
    uint32_t i = 0;
