...
void create_chunk_buffers_greedy();
...
void generate_chunk() {
    ...
    FastNoise::SmartNode<> fnGenerator = fnFractal;

    std::vector<float> noiseOutput(16 * 16 * 16);

    fnGenerator->GenUniformGrid3D(noiseOutput.data(), 0, 0, 0, 16, 16, 16, 0.1f, 1337);
}

// Build and upload a greedy-meshed 16x16x16 chunk
void create_chunk_buffers_greedy() {
    constexpr int SX = 16, SY = 16, SZ = 16;
    auto idx3 = [&](int x,int y,int z){ return x + SX*(y + SY*z); };

    // Simple voxel data: solid ground at y==0
    std::vector<uint8_t> vox(SX*SY*SZ, 0);
    for (int z=0; z<SZ; ++z)
        for (int y=0; y<SY; ++y)
            for (int x=0; x<SX; ++x)
                vox[idx3(x,y,z)] = 1;
    ...
    struct Tile { int x,y; } faceTiles[6] = {
        {0,11}, // +Z Front
        {0,11}, // +X Right
        {0,11}, // -Z Back
        {0,11}, // -X Left
    ...
    auto emit_quad = [&](int face, // 0:+Z,1:+X,2:-Z,3:-X,4:+Y,5:-Y
                          float x0,float y0,float z0,
                          float x1,float y1,float z1,
                          float x2,float y2,float z2,
                          float x3,float y3,float z3) {
        // Center the chunk around origin
        float cx = SX*0.5f, cy = SY*0.5f, cz = SZ*0.5f;
        struct P{float x,y,z;};
        P p[4] = {{x0-cx,y0-cy,z0-cz},{x1-cx,y1-cy,z1-cz},{x2-cx,y2-cy,z2-cz},{x3-cx,y3-cy,z3-cz}};

        const uint16_t tilePacked = PackTile(faceTiles[face].x, faceTiles[face].y);
        const uint16_t packedLight = PackLight(15, 15, 0);
        const uint32_t materialFlags = (uint32_t)(faceTiles[face].x | (faceTiles[face].y<<8));
        const int cornerU[4] = {0,1,1,0};
        const int cornerV[4] = {1,1,0,0};

        uint16_t base = (uint16_t)verts.size();
        for (int i=0;i<4;++i){
            uint16_t uvFace = PackUVFace(cornerU[i], cornerV[i], face);
            VertexPacked v{};
            v.pos.x = p[i].x; v.pos.y = p[i].y; v.pos.z = p[i].z;
            v.materialFlags = materialFlags;
            v.tilePacked = (uint32_t)tilePacked;
            v.packedLightUV = (uint32_t)packedLight | ((uint32_t)uvFace<<16);
            verts.push_back(v);
        }
        // two triangles (CCW), using order 0,1,2, 0,2,3
    ...
    // Greedy meshing over each axis
    // Based on Mikola Lysenko's algorithm
    auto mesh_axis = [&](int axis){
        int u = (axis+1)%3;
        int v = (axis+2)%3;
        int dims[3] = {SX, SY, SZ};
        std::vector<int> idMask(dims[u]*dims[v]);
        std::vector<int> sideMask(dims[u]*dims[v]); // +1 for positive normal, -1 for negative

        auto get = [&](int x,int y,int z)->int{
            if (x<0||y<0||z<0||x>=SX||y>=SY||z>=SZ) return 0;
            return vox[idx3(x,y,z)]!=0;
        };

        for (int d=0; d<=dims[axis]; ++d){
            // Build mask between layer d-1 and d
            int n = 0;
            for (int j=0;j<dims[v];++j){
                for (int i=0;i<dims[u];++i){
                    int x[3] = {0,0,0};
                    x[axis]=d; x[u]=i; x[v]=j;
                    int x1[3] = {x[0],x[1],x[2]}; x1[axis]=d-1;
                    int a = (d>0) ? get(x1[0],x1[1],x1[2]) : 0;
                    int b = (d<dims[axis]) ? get(x[0],x[1],x[2]) : 0;
                    if (a!=b){
                        idMask[n] = 1; // single material id for now
                        sideMask[n] = (b?+1:-1); // positive if face points towards +axis
                    } else {
                        idMask[n] = 0; sideMask[n] = 0;
                    }
                    ++n;
                }
            }
            ...
            for (int j=0;j<dims[v];){
                for (int i=0;i<dims[u];){
                    int id = idMask[n]; int sgn = sideMask[n];
                    if (!id){ ++i; ++n; continue; }
                    // compute width
                    int w=1; while (i+w<dims[u] && idMask[n+w]==id && sideMask[n+w]==sgn) ++w;
                    // compute height
                    int h=1; bool done=false;
                    while (j+h<dims[v] && !done){
                        for (int k=0;k<w;++k){
                            int idx = n + k + h*dims[u];
                            if (!(idMask[idx]==id && sideMask[idx]==sgn)){ done=true; break; }
                        }
                        if (!done) ++h;
                    }
                    // clear mask
                    for (int dj=0; dj<h; ++dj){
                        for (int di=0; di<w; ++di){
                            idMask[n + di + dj*dims[u]] = 0;
                            sideMask[n + di + dj*dims[u]] = 0;
                        }
                    }
                    ...
                    int x0[3]={0,0,0}; x0[axis]=d; x0[u]=i; x0[v]=j;
                    // convert to positions in voxel grid
                    float fx = (float)x0[0], fy=(float)x0[1], fz=(float)x0[2];
                    float fx_du = fx + du[0]; float fy_du = fy + du[1]; float fz_du = fz + du[2];
                    float fx_dv = fx + dv[0]; float fy_dv = fy + dv[1]; float fz_dv = fz + dv[2];
                    float fx_du_dv = fx + du[0] + dv[0];
                    float fy_du_dv = fy + du[1] + dv[1];
                    float fz_du_dv = fz + du[2] + dv[2];

                    if (sgn>0){
                        // Face with normal +axis at position d
                        if (axis==0){ // +X
                            emit_quad(1, fx,      fy,      fz,
                                         fx,      fy,      fz_du,
                                         fx,      fy_dv,   fz_du,
                                         fx,      fy_dv,   fz);
                        } else if (axis==1){ // +Y
                            emit_quad(4, fx,      fy,      fz_du,
                                         fx_du,   fy,      fz_du,
                                         fx_du,   fy,      fz,
                                         fx,      fy,      fz);
                        } else { // +Z
                            emit_quad(0, fx,      fy,      fz,
                                         fx_du,   fy,      fz,
                                         fx_du,   fy_dv,   fz,
                                         fx,      fy_dv,   fz);
                        }
                    } else {
                        // Face with normal -axis at position d-1
                        if (axis==0){ // -X
                            float x = fx; // already at d
                            emit_quad(3, x,       fy,      fz_du,
                                         x,       fy,      fz,
                                         x,       fy_dv,   fz,
                                         x,       fy_dv,   fz_du);
                        } else if (axis==1){ // -Y
    ...
    if (!verts.empty() && !inds.empty()){
        // VB
        D3D11_BUFFER_DESC vbDesc = {};
        vbDesc.ByteWidth = (UINT)(verts.size()*sizeof(VertexPacked));
        vbDesc.Usage = D3D11_USAGE_DEFAULT;
        vbDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
        D3D11_SUBRESOURCE_DATA vinit = { verts.data(), 0, 0 };
        HRESULT hr = g_RendererContext->Device->CreateBuffer(&vbDesc, &vinit, &g_RendererContext->VertexBuffer);
        SM_ASSERT(SUCCEEDED(hr), "Failed to create chunk VB");

        // IB
        D3D11_BUFFER_DESC ibDesc = {};
        ibDesc.ByteWidth = (UINT)(inds.size()*sizeof(uint16_t));
        ibDesc.Usage = D3D11_USAGE_DEFAULT;
        ibDesc.BindFlags = D3D11_BIND_INDEX_BUFFER;
        D3D11_SUBRESOURCE_DATA iinit = { inds.data(), 0, 0 };
        hr = g_RendererContext->Device->CreateBuffer(&ibDesc, &iinit, &g_RendererContext->IndexBuffer);
        SM_ASSERT(SUCCEEDED(hr), "Failed to create chunk IB");

        g_RendererContext->IndexCount = (uint32_t)inds.size();
    } else {
        g_RendererContext->IndexCount = 0;
    }

    // Create CBs if not yet
    if (!g_RendererContext->PerFrameCB){
        D3D11_BUFFER_DESC perFrameDesc = {};
        perFrameDesc.ByteWidth = sizeof(PerFrameCBData);
        perFrameDesc.Usage = D3D11_USAGE_DYNAMIC;
        perFrameDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
        perFrameDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
        HRESULT hr = g_RendererContext->Device->CreateBuffer(&perFrameDesc, nullptr, &g_RendererContext->PerFrameCB);
    ...
    if (!g_RendererContext->PerDrawCB){
        D3D11_BUFFER_DESC perDrawDesc = {};
        perDrawDesc.ByteWidth = sizeof(PerDrawCBData);
        perDrawDesc.Usage = D3D11_USAGE_DYNAMIC;
        perDrawDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
        perDrawDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
        HRESULT hr = g_RendererContext->Device->CreateBuffer(&perDrawDesc, nullptr, &g_RendererContext->PerDrawCB);
...
void render(RenderData* renderData, pfnRenderUIOverlay uiOverlay) {
    ...
    // Draw indexed chunk
    if (g_RendererContext->IndexCount > 0) {
        g_RendererContext->DeviceContext->DrawIndexed(g_RendererContext->IndexCount, 0, 0);
    }
